...
![Scoreboard Preview](https://via.placeholder.com/800x200?text=Scoreboard+Preview+Image) 
*(Replace this link with a screenshot of your overlay if you have one)*
...
| **`control.html`** | **The Controller.** Open in Chrome/Edge to manually score the game and edit team names/logos. |
| **`gameconfig.json`** | **The Settings.** Stores persistent data like team names, records, colors, and logo paths. |
| **`skordle.ps1`** | **The Scraper.** A PowerShell script that pulls scores from the web for the ticker. |
...
6.  **⚠️ IMPORTANT:** To save these settings permanently, click **Download JSON**. This will download a new `gameconfig.json` file. You must **overwrite/replace** the existing file in your scoreboard folder with this new one. (Browsers cannot silently overwrite files for security reasons).
...
## 🏎️ Setting up the Automated Ticker (Optional)
If you want the scrolling ticker at the bottom, you need to set up the scraper.

### A. Download ChromeDriver
The script needs a driver to interface with Google Chrome.
...
4.  Extract `chromedriver.exe` into a folder (e.g., `C:\Scoreboard\chromedriver\`).

### B. Configure the Script
1.  Open `skordle.ps1` in Notepad.
2.  Update the `$driverPath` to point to your `chromedriver.exe`.
3.  Update the `$outputFile` to point to where you want the CSV saved.

### C. Run the Scraper
1.  Right-click `skordle.ps1` > **Run with PowerShell**.
2.  If successful, `SkordleScores.csv` will be updated with live data.
3.  *Tip: Use Windows Task Scheduler to run this script every 5 minutes automatically.*
